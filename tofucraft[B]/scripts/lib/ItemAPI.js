import { Block, GameMode, ItemStack } from "@minecraft/server";
...
export class ItemAPI {
     ...
     */
    static toItemStack(item, amount = 1) {
     ...
     * @returns 是否清除成功
     */
    static clear(player, slot, amount = 1) {
        if (this.isCreative(player))
            return true;
        const container = this.getContainer(player);
        if (!container)
     ...
     */
    static spawn(target, item, amount = 1, location) {
     ...
     * @returns 生成坐标
     */
    static getSpawnLocation(target) {
     ...
     * @returns 是否添加成功
     */
    static add(player, item, amount = 1) {
        const container = this.getContainer(player);
        if (!container)
            return undefined;
        const itemStack = this.toItemStack(item, amount);
